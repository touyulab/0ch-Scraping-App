...
class newResponseView: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        // response 
        // >>1 など
        textView.text = "\(self.delegate.response)"
    ...
    @IBAction func AddResponse(sender: UIButton) {
        
        let name = textField1.text!
        let mail = textField2.text!
        let body = textView.text
        
        let post = "mail=\(mail)&name=\(name)&body=\(body)"
